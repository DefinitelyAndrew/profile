...
export const videoOptions = {
  // ---- Color / tone ----
  brightness: 0.82,       // 1.0 = normal, 1.2 = brighter, 0.7 = darker
  contrast: 1.02,         // 1.0 = normal, 1.15 = more punchy, 0.9 = softer
  saturate: 0.95,         // 1.0 = normal, 1.3 = vivid, 0 = grayscale
  hueRotate: 0,           // degrees (0 = no shift)

  // ---- Focus / clarity ----
  blur: 0,                // px — 0 = sharp, 2 = hazy
  sharpen: 0,             // 0..1 — fake sharpen via extra contrast + saturation

  // ---- Dark gradient on top of the video (0..1) ----
  overlayDarkness: 0.25,  // top-bottom gradient strength
  overlaySide: 0.15,      // left-side darkening strength

  // ---- Film grain overlay ----
  grainOpacity: 0.05,     // 0 = no grain, 0.15 = heavy grain
};

/** Builds the CSS filter string for the <video> element */
export function buildVideoFilter(): string {
  const { brightness, contrast, saturate, blur, hueRotate, sharpen } =
    videoOptions;

  // Simulated sharpen — real sharpen needs SVG feConvolveMatrix,
  // but pushing contrast + saturation gives a similar "crisper" feel.
...
/** Builds the gradient for the overlay div on top of the video */
export function buildOverlay(): string {
  const { overlayDarkness, overlaySide } = videoOptions;
  return [
    `linear-gradient(180deg, rgba(25, 25, 30, 0.15) 0%, rgba(25, 25, 30, ${overlayDarkness}) 100%)`,
    `linear-gradient(90deg, rgba(25, 25, 30, ${overlaySide}) 0%, rgba(25, 25, 30, 0) 60%)`,
  ].join(", ");
}
...
export const uiOptions = {
  ...
  /** Gap between the letters of "cosmin", in em.
   *  Negative values pull letters tighter, positive spreads them out.
   *  Default = -0.01em (letters almost touching, as-is). */
...
export const aboutSectionOptions = {
  ...
  heyImFontSize: 18,
  /** How much the "cosmin." name overlaps "hey, i'm" (negative = pulls name up).
...
/* =========================================================
   Others section options
   ========================================================= */
export const othersOptions = {
  /** Scale multiplier for the 88×31 web buttons.
   *  1 = native size (88×31 px), 1.5 = 132×46 px, 2 = 176×62 px. */
  buttonScale: 1.1,

  /** Number of button columns in the grid. */
  buttonColumns: 3,

  /** Font size (px) of the COPY LINK / COPIED! overlay text. */
